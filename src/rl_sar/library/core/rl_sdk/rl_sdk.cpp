...
void RL::UpdateContactFilter()
{
    ...
    for (int i = 0; i < 4; ++i) {
        current_contacts[i] = (std::abs(this->foot_forces[i]) > contact_threshold);
    }

    for (int i = 0; i < 4; ++i) {
        this->obs.contact_filt[i] = current_contacts[i] || this->last_contacts[i];
    }

    this->last_contacts = current_contacts;
}
